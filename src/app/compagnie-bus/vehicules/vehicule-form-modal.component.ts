          ...
          <form [formGroup]="form" (ngSubmit)="submit()">
            <div class="grid">
              <label>
                ...
                <input type="text" formControlName="immatriculation" placeholder="Ex: DK-1234" />
              </label>
              <div class="type-indicator">
                <span class="type-label">Type de véhicule</span>
                <span class="type-value">Bus</span>
              </div>
              <label>
            ...
            <div class="modal-actions">
              ...
              <button type="submit" class="btn-primary" [disabled]="form.invalid || isSubmitting || !compagnieTrackingId">
                {{ isSubmitting ? 'Enregistrement...' : 'Créer le véhicule' }}
              </button>
            </div>
          </form>
        </div>
...
export class VehiculeFormModalComponent implements OnInit, OnChanges, OnDestroy {
  private readonly fb = inject(FormBuilder);
  @Input() isOpen = false;
  @Output() closed = new EventEmitter<void>();
  @Output() saved = new EventEmitter<VehiculeResponse>();

  readonly vehicleTypeLabel = 'Bus';
  ...
  loadCompagnie(): void {
    ...
    const sub = this.compagnieService.getByProprietaire(currentUser.trackingId).subscribe({
      ...
      error: (error) => {
        console.error('[VehiculeFormModal] Erreur chargement compagnie', error);
  ...
  submit(): void {
    ...
    const sub = this.vehiculeService.create(payload).subscribe({
      ...
      error: (error) => {
        console.error('[VehiculeFormModal] Erreur création véhicule', error);
        this.isSubmitting = false;
        this.submissionError = error?.error?.message || "Erreur lors de la création du véhicule.";
      }
    });
