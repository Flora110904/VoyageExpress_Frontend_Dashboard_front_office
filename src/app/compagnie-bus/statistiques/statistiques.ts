...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, CompagnieBusNavbarComponent],
...
export class Statistiques implements OnInit, OnDestroy {
  ...
  reservationsByMonth: MonthlyMetric[] = [];
  revenuesByMonth: MonthlyRevenue[] = [];
  ...
  private loadStats(): void {
    ...
    const sub = this.reservationService.getStatistiques().subscribe({
      ...
      error: (err) => {
        console.error('Erreur lors du chargement des statistiques bus:', err);
  ...
  private buildCountSeries(metrics?: Record<string, number> | null): MonthlyMetric[] {
    if (!metrics) {
      return [];
    }
  ...
  private buildAmountSeries(metrics?: Record<string, number> | null): MonthlyRevenue[] {
    if (!metrics) {
      return [];
    }
